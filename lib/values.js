...
export default {
  ...
  execDMP(dumpFile,config,solves) {
    const gamsParams = config.gamsexe + ' "' + dumpFile.path + '" suppress=1 PS=0 lo=3 -scrdir="' + config.scratchdir + '" '
  ...
  getData(lst,config,solves) {
    ...
    function save(type,solve,symbol,data) {
      ...
      if (!global.gamsSolves) {
        global.gamsSolves = [statement]
      } else if (!_.find(global.gamsSolves,{'line': Number(solve)})) {
