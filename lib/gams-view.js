...
export default class StructureView {
  ...
  constructor() {
      ...
      methods: {
          ...
          this.fileWatchers = atom.project.getPaths().forEach(project => {
            watchPath(project,{}, events => {
              for (const event of events) {
                ...
                this.parseListing(event.path, true)
              }
            })
          })
        ...
        updateVue(symFound) {
          ...
          if(symbol) {
            ...
            if(symbol.data && atom.workspace.getBottomDock().isVisible()) {
              this.consolePanel.clear()
              this.consolePanel.stickTop()
              if (global.gamsUpdating && global.gamsUpdating.a) this.consolePanel.log('Values are being updated...')
              else this.consolePanel.log(symbol.data)
            }
          }
        },
    ...
    global.gamsUpdating = new Proxy({}, handler)
  }
