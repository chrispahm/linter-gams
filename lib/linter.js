...
export default {
  ...
  compile (config, callback) {
    ...
    findUp(entryFiles, {cwd: config.directory}).then((expStarter) => {
      if (expStarter && path.parse(expStarter).base !== '.gamslintc.js') {
      ...
      } else if (expStarter && path.parse(expStarter).base === '.gamslintc.js') {
        const configFile = require(expStarter)
        config.directory = path.dirname(expStarter)
        config.filePath = configFile.entry ? path.resolve(config.directory,configFile.entry) : config.filePath
        parameters = configFile.compParams || parameters
      }
