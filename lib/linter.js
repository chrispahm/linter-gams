...
function compile (config, callback) {
	...
	findUp(['exp_starter.gms', 'capmod.gms', 'com_.gms'], {cwd: config.directory}).then((expStarter) => {
		...
		console.log(`${config.gamsexe} ${config.filePath} action=c o=${nullOutput} ${tagsParam} fErr=${config.scratchdir}${path.sep}${randStr}.err -workdir="${config.directory}" -curDir="${config.directory}" ${ggigParams}`)
		shell.exec(`${config.gamsexe} ${config.filePath}  action=c o=${nullOutput} ${tagsParam} fErr=${config.scratchdir}${path.sep}${randStr}.err -workdir="${config.directory}" -curDir="${config.directory}" ${ggigParams}`, function(code, stdout, stderr) {
			console.log(stdout)
			if (stderr) {
