...
function compile (config, callback) {
	...
	let ggigParams = ''
	findUp('exp_starter.gms', config.directory).then((expStarter) => {
		if (expStarter) {
		...
		shell.exec(`${config.gamsexe} ${config.filePath} lo=0 action=c o=${nullOutput} fErr=${config.scratchdir}${randStr}.err -scrdir="${config.scratchdir}" -workdir="${config.directory}" -curDir="${config.directory}" ${ggigParams}`, function(code, stdout, stderr) {
			if (stderr) {
