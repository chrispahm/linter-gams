...
export default {
  ...
  compile (config, callback) {
    ...
    findUp(entryFiles, {cwd: config.directory}).then((expStarter) => {
      ...
      shell.exec(`${config.gamsexe} "${config.filePath}" lo=0 action=c o=${nullOutput} fErr="${config.scratchdir}${path.sep}${randStr}.err" dumpopt=11 rf=${config.scratchdir}${path.sep}${randStr}.ref -scrdir="${config.scratchdir}" -workdir="${config.directory}" -curDir="${config.directory}" ${ggigParams}`, function(code, stdout, stderr) {
        if (stderr) {
