...
export default {
  ...
  compile (config, callback) {
    ...
    findUp(['exp_starter.gms', 'capmod.gms', 'com_.gms'], {cwd: config.directory}).then((expStarter) => {
      ...
      shell.exec(`${config.gamsexe} ${config.filePath}  action=c o=${nullOutput} rf=${config.scratchdir}${path.sep}${randStr}.ref fErr=${config.scratchdir}${path.sep}${randStr}.err -workdir="${config.directory}" -curDir="${config.directory}" ${ggigParams}`, function(code, stdout, stderr) {
        if (stderr) {
