...
export default {
  ...
  compile (config, callback) {
    ...
    findUp(entryFiles, {cwd: config.directory}).then((entryFile) => {
      ...
      const gamsProcess = shell.exec(`${config.gamsexe} "${config.filePath}" lo=3 a=c o="${config.scratchdir}${path.sep}${randStr}.lst" fErr="${config.scratchdir}${path.sep}${randStr}.err" ${dumpOpt} rf="${config.scratchdir}${path.sep}${randStr}.ref" -scrdir="${config.scratchdir}" -workdir="${config.directory}" -curDir="${config.directory}" ${parameters}`, {async: true})
