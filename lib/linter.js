...
function parse(listing, gamsFile) {
	...
	// temp workaround to solve multiple errors being displayed
	if (errFile !== gamsFile || !message.excerpt) return []
	return [message]
}

const lint = (args) => {
	const config = args[0]
	const gamsFile = args[1]
	return new Promise((resolve) => {
		return compile(config, (listing) => {
			if (listing.split(/\n/).length <= 2) { return resolve([]) }
			return resolve(parse(listing, gamsFile))
		})
	})
}
