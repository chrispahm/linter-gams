...
function getConfig (directory, gamsFile) {
	return new Promise((resolve) => {
		if (fs.existsSync(`${directory}/linter-gams.config.js`)) {
			...
			config.callFile = gamsFile
			if (config.ggig) resolve(parseGGIGparams(config, gamsFile))
			return resolve([config, gamsFile])
		}
			...
			config.callFile = gamsFile
			if (config.ggig) resolve(parseGGIGparams(config, gamsFile))
			resolve([config, gamsFile])
		}
		else {
			resolve({'entry': gamsFile, 'directory': directory, 'scratchdir': __dirname}, gamsFile)
		}
	})
}

function parseGGIGparams (config, gamsFile) {
	return new Promise ((resolve) => {
