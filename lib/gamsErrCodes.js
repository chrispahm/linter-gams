module.exports = {
	...
	'210': 'Unknown dollar control option. valid are -  on/offmargin   on/offdollar  on/offsymlist  on/offuellist  on/offsymxref  on/offuelxref  on/offinline   on/offnestcom  on/offeolcom   on/offinclude  on/offwarning  on/offsparse  on/offorder    on/offput  single double eject (listing control)  on/offlisting  on/offecho  onmulti offmulti    (multiple declarations for set table param)  onupper offupper    (show source text as entered)  ontext offtext      (bracket a block of comment lines)  ondigit offdigit    (off ignores precision on input)  lines               (followed by line count)  title stitle        (followed by new title/subtitle)  mincol maxcol       (followed by column position)  dollar comment      (followed by 1 character)  hidden              (followed by unlisted comment)  phantom             (followed by an element)  inlinecom           (followed by delimiter spec eg. /* */)  eolcom              (followed by delimiter spec eg. _$)  verid               (followed by version ident eg tp5-am-002)  include             (followed by filename)  bat/sysinclude      (followed by filename and parameters)',
