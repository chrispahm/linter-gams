  ...
  getGamsPath() {
    ...
    // if not found, check in standard directories
      ...
      // and mac
        ...
        const checkOld = glob.sync('/Applications/GAMS*/sysdir/gams')
        const checkNew = glob.sync('/Applications/GAMS*/Resources/sysdir/gams')
        if (checkOld.length > 0) {
          gamsExec = checkOld[checkOld.length - 1]
          return gamsExec
        }
        else if (checkNew.length > 0) {
          gamsExec = checkNew[checkNew.length - 1]
          return gamsExec
        } else {
          this.alert('noGAMS')
          return ''
