...
export default function ListingParser(file, callback) {
	...
	function save(type, pos, push, entries) {
		...
		if (entries) {
			if (entry.entries) entry.entries.push(entries)
	...
	rl.on('line', (line) => {
		...
		} else if (line === 'E x e c u t i o n') {
